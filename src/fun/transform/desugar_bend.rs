  ...
  pub fn desugar_bend(&mut self) -> Result<(), Diagnostics> {
    ...
    for def in self.book.defs.values_mut() {
      ...
      for rule in def.rules.iter_mut() {
        if let Err(err) = rule.body.desugar_bend(&def.name, &mut fresh, &mut new_defs, builtin, def.check) {
  ...
  fn desugar_bend(
    ...
    maybe_grow(|| {
      ...
      for child in self.children_mut() {
        child.desugar_bend(def_name, fresh, new_defs, builtin, check)?;
      }
      ...
      if let Term::Bend { .. } = self {
        ...
        *fresh += 1;
        ...
        let def = Definition::new_gen(new_nam.clone(), vec![Rule { pats: vec![], body }], builtin, check);
