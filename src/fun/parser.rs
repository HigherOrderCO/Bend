use crate::{
  fun::{
    ...
    MatchRule, Name, Num, Op, Pattern, Rule, Source, Tag, Term, Type, STRINGS,
  },
...
impl<'a> FunParser<'a> {
  pub fn new(input: &'a str, builtin: bool) -> Self {
    Self { input, index: 0, builtin }
  }
  ...
  pub fn parse_book(&mut self, default_book: ParseBook) -> ParseResult<ParseBook> {
    ...
    while !self.is_eof() {
      ...
      if self.starts_with_keyword("object") {
        ...
        let mut prs = ImpParser { input: self.input, index: *self.index(), builtin: self.builtin };
        let (adt, nxt_indent) = prs.parse_object(indent)?;
        self.index = prs.index;
        let end_idx = *self.index();
        self.add_type_def(adt, ini_idx..end_idx, &mut book)?;
        indent = nxt_indent;
        continue;
      }
  ...
  fn parse_type_def(&mut self) -> ParseResult<Adt> {
    ...
    let source = if self.builtin { Source::Builtin } else { Source::Local(ini_idx..end_idx) };
  ...
  /// Parse the left hand side of a rule, up to the '='.
  ...
  /// Lookahead to check if the next top level is a pattern-matching rule with the given name.
  ...
  fn add_type_def(&mut self, adt: Adt, span: Range<usize>, book: &mut ParseBook) -> ParseResult<()> {
    ...
    for ctr in adt.ctrs.keys() {
      if let Some(builtin) = book.contains_builtin_def(ctr) {
        return Err(FunParser::redefinition_of_function_msg(builtin, ctr));
      }
